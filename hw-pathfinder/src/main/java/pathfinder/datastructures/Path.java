...
/**
 * This represents an immutable path between two cartesian coordinate T, particularly
...
public class Path<T> implements Iterable<Path<T>.Segment> {
    ...
    // AF(this) =
    //      first coordinate in the path => start
    //      each "step" along the path between coordinates => elements of list path, where
    //          path.get(0) is the first step from the start coordinate to an intermediate coordinate, and
    //          path.get(path.size() - 1) is the final step from an intermediate coordinate to the end
    //      total cost along the path => cost
    //      the destination coordinate in this path, opposite the start coordinate => getEnd()
    ...
    /**
     * The coordinate at the beginning of this path.
     */
    private T start;

    /**
     * The ordered sequence of segments representing a path between coordinates.
     */
    private List<Segment> path;

    /**
     * Creates a new, empty path containing a start coordinate. Essentially this represents a path
     * from the start coordinate to itself with a total cost of "0".
     *
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last coordinate
     ...
     * segmentCost}.
     *
     * @param newEnd      The coordinate being added at the end of the segment being appended to this path
    ...
    /**
     * @return The coordinate at the beginning of this path.
     */
    public T getStart() {
        return start;
    }
    ...
    /**
     * @return The coordinate at the end of this path, which may be the start coordinate if this path
     * contains no segments (i.e. this path is from the start coordinate to itself).
     */
    public T getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * coordinate and ending at the end coordinate. In the case that this path represents a path between
     * the start coordinate and itself, this iterator contains no elements. This iterator does not
     ...
     * both paths are empty, they are only equal if their starting coordinate is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between coordinates.
    ...
    public class Segment {
         ...
         *
         * @param start The starting coordinate of this segment.
         * @param end   The ending coordinate of this segment.
        ...
        private Segment(T start, T end, double cost) {
            if(start == null || end == null) {
                throw new NullPointerException("Segments cannot have null coordinates.");
            }
        ...
        /**
         * @return The beginning coordinate of this segment.
         */
        public T getStart() {
            // Note: Since coordinates are immutable, this isn't rep exposure.
            return this.start;
        }
